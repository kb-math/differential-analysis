...
To see how this demonstrates the symmetries of the Laplacian and D'Alembertian, we observe the following. First, notice that the Laplacian and D'Alembertian of a function $f$ may be written as the trace of $L_{D^2f(x)}$ with respect to the Euclidean norm and Minkowski pseudo norm, respectively. On the other hand, by Lemma REF we have that $$D^2(f \circ Q) (x) = D^2(f(Qx)) \circ Q^2,$$ i.e., $D^2(f \circ Q) (x) (v_1,v_2) = D^2(f(Qx)) (Qv_1, Qv_2)$, so the corollary applies.
